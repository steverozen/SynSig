...
#' Read Catalog Functions TODO(Steve): these are really read signature functions.
#' There is not custom catalog reading function for SignatureAnalyzer. RENAME!
#'
...
#'
#' \code{ReadSACatCOMPOSITE} Read a COMPOSITE catalog, which is an rbind
#' of a 1536 catalog, a DBS catalog, and an ID catalog. This function does
#' not read catalogs as generated by SignatureAnalyzer, but rather
...
#' @export
ReadSACatCOMPOSITE <- function(path) {
  retval <- read.csv(path, header = T, row.names = 1)
  return(as.matrix(reval))
}
...
#' Run SignatureAnalyzer on a single data set and put results
...
#' in specified location.
#'
#' The 4 coordinated data set are termed sa.sa.COMPOSITE, sa.sa.96, sp.sa.COMPOSITE
#' and sp.sp, described elsewhere.
#'
#' @param signatureanalyzer.code.dir The directory holding the
#' SignatureAnalyzer code.
#'
...
#' its only argument and returning a catalog as a numeric matrix.
#'
#' @param dir.root Root of directory tree that contains the input data and to
...
#'
#' @export
SignatureAnalyzerRunOn4MatchedDataSets <-
  function(signatureanalyzer.code.dir,
           read.catalog.function,
           dir.root,
           write.signature.function,
    ...
    subdirs <- c("sa.sa.COMPOSITE", "sp.sa.COMPOSITE", "sa.sa.96", "sp.sp")
    read.fn <- c(ReadSACatCOMPOSITE, ReadSACatCOMPOSITE, ReadSACat96, ReadSACat96)
    write.fn <- c(WriteCatCOMPOSITE, WriteCatCOMPOSITE, WriteCat96, WriteCat96)
    exposure <- c("sa.exposure.csv", "sp.exposure.csv",
                  "sa.exposure,csv", "sp.exposure.csv")

    tmp.fn <- function(subdir, read.fn, write.fn, exposure) {
      retval1 <-
        SignatureAnalyzerReplicateRuns(
          num.runs = 20,
          ...
          out.dir = paste0(dir.root, "/", subdir, "/SA.output/"),
