...
GenerateSynFromReal <- function(real.exp, num.syn.tumors, file.prefix) {
  parms <- GetSynSigParamsFromExposures(sa.rcc.exp)
  
  froot <- OutFile(file.prefix)
  
  WriteExposure(sa.rcc.exp, paste0(froot, "real-exp.csv"))
  
  parm.file <- paste0(froot, "parms.csv")
  WriteSynSigParams(parms, parm.file)

  syn.exp <- 
    GenerateSyntheticExposures(parms, num.syn.tumors, "RCC")
  ...
  # sa.check.param should be similar to sa.rcc.parms
...
sa.rcc.info <-
  GenerateSynFromReal(sa.rcc.exp, num.syn.tumors, "sa-rcc-")
sp.rcc.info <-
  GenerateSynFromReal(sp.rcc.exp, num.syn.tumors, "sp-rcc-")

```

### 1.2 Synthetic uterine adenocarcinoma exposures
...
sa.ute.info <-
  GenerateSynFromReal(sa.ute.exp, num.syn.tumors, "sa-ute-")
sp.ute.info <-
  GenerateSynFromReal(sp.ute.exp, num.syn.tumors, "sp-ute-")

```

TODO Next: create uterine addeno, exposures, then merge exposures
