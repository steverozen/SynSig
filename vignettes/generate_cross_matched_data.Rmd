...
```{r}
out.data.dir <<- "syn_3_5_40_rcc_and_ovary_v2/"
set.seed(191905)
num.syn.tumors <- 500
...
WriteExposure(sa.rcc.ova.syn.exp, OutDir("sa.rcc.ova.syn.exposure.csv"))
WriteExposure(sp.rcc.ova.syn.exp, OutDir("sp.rcc.ova.syn.exposure.csv"))
...
WriteCatCOMPOSITE(sa.sa.COMPOSITE,
                  OutDir("sa.sa.COMPOSITE.RCC.and.OVA.syn.data.csv"))

sa.sa.96 <- GenSynCatalogs(sa.96.sigs, sa.rcc.ova.syn.exp)

WriteCat96(sa.sa.96, OutDir("sa.sa.96.RCC.and.OVA.syn.data.csv"))
```
...
WriteCatCOMPOSITE(sp.sa.COMPOSITE,
                  OutDir("sp.sa.COMPOSITE.RCC.and.OVA.syn.data.csv"))
...
WriteCat96(sp.sp, OutDir("sp.sp.RCC.and.OVA.syn.data.csv"))
```
...
out.data.dir <- "syn_3_5_40_abst_v2/"
set.seed(44)
num.syn.tumors <- 1000

sp.abst.info <-
  GenerateSynAbstract(
    x.sp.parms, num.syn.tumors, "sp-3-5-40-abst-", "SP_Syn_Abst")

sa.abst.info <-
  GenerateSynAbstract(
    x.sa.parms,
    num.syn.tumors, "sa-3-5-40-abst-", "SA_Syn_Abst")

# We will need the exposures later when evaluating the attributed signatures
WriteExposure(sp.abst.info$syn.exp, OutDir("sp.abst.syn.exposure.csv"))
WriteExposure(sp.abst.info$syn.exp, OutDir("sa.abst.syn.exposure.csv"))
```
...
WriteCatCOMPOSITE(sa.sa.COMPOSITE,
                  OutDir("sa.sa.COMPOSITE.abst.syn.data.csv"))

sa.sa.96 <- GenSynCatalogs(sa.96.sigs, sa.abst.info$syn.exp)

WriteCat96(sa.sa.96, OutDir("sa.sa.96.abst.syn.data.csv"))
```
...
WriteCatCOMPOSITE(sp.sa.COMPOSITE,
                  OutDir("sp.sa.COMPOSITE.abst.syn.data.csv"))

sp.sp <- GenSynCatalogs(sp.sigs, sp.abst.info$syn.exp)

WriteCat96(sp.sp, OutDir("sp.sp.abst.syn.data.csv"))
