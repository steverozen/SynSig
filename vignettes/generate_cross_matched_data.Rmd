...
```{r}
out.data.dir <<- "syn_3_5_40_rcc_and_ovary/"  # Must be a directory; be sure to end in "/"
set.seed(191905)
num.syn.tumors <- 500
...
sp.sa.map.info <-
  MapSPToSASignatureNamesInExposure(sp.syn.exp, sp.sigs, sa.96.sigs)
sp.sa.COMPOSITE <- 
...
```{r}
sp.sp <- GenSynCatalogs(sp.sigs, sp.syn.exp)
...
out.data.dir <- "syn_3_5_40_abst/"  # Will be a directory
set.seed(44)
num.syn.tumors <- 1000
...
sa.abst.info <-
  GenerateSynAbstract(
    x.sa.parms,
    num.syn.tumors, "sa-3-5-40-abst-", "SP_Syn_Abst")
```
...
```{r}
tmp.exp <- copy(sp.abst.info$syn.exp)
